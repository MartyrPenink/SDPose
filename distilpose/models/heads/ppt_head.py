...
from .utils.tokenbase import TokenPose_PPT_base

BN_MOMENTUM = 0.1

@HEADS.register_module()
class PPTTokenPoseHead(nn.Module):
    """
    ...
    def __init__(self,
        ...
        if loss_vit_dist is not None:
            self.vit_dist_loss = build_loss(loss_vit_dist)
        else:
            self.vit_dist_loss = None
        ...
        self.tokenpose = TokenPose_PPT_base(feature_size=tokenpose_cfg.feature_size, 
    ...
    def get_loss(self, output, target, target_weight):
        ...
        device = student_vis_token.device
        losses['heatmap_loss'] = self.keypoint_loss(output.pred, target, target_weight)
